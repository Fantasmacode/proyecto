...
<div class="form-group">
	...
	<select class="form-control {{ $errors-> has('id_comercio')?'is-invalid':''}}" name="id_comercio" id="comercio" value="{{ isset ($admin->id_comercio)?$admin->id_comercio:old('id_comercio') }}" aria-label="Default select example">
		...
		<option value="{{$comercio->id_comercio}}" {{ old('id_comercio') == $comercio->id_comercio || $admin->id_comercio == $comercio->id_comercio ? 'selected' : '' }}>{{$comercio->tipo_comercio}}</option>
		@endforeach
	</select>
...
<div class="form-group">
	...
	<select class="form-control {{ $errors-> has('id_raz')?'is-invalid':''}}" name="id_raz" id="raza" value="{{ isset ($admin->id_raz)?$admin->id_raz:old('id_raz') }}" aria-label="Default select example">
		...
		<option value="{{$raza->id_raz}}" {{ old('id_raz') == $raza->id_raz || $admin->id_raz == $raza->id_raz ? 'selected' : '' }}>{{$raza->nombre_raz}}</option>
		@endforeach
	</select>
...
<div class="form-group">
	...
	<select class="form-control {{ $errors-> has('finalidad_bovino')?'is-invalid':''}}" name="finalidad_bovino" id="finalidad" aria-label="Default select example">
		...
		<option value="Carne" {{ old('finalidad_bovino') == "Carne" || $admin->finalidad_bovino == "Carne" ? 'selected' : '' }}>Carne</option>
		<option value="Leche" {{ old('finalidad_bovino') == "Leche" || $admin->finalidad_bovino == "Leche" ? 'selected' : '' }}>Leche</option>
		<option value="Doble Propósito" {{ old('finalidad_bovino') == "Doble Propósito" || $admin->finalidad_bovino == "Doble Propósito" ? 'selected' : '' }}>Doble Propósito</option>
	</select>
...
<div class="form-group">
	...
	<select class="form-control {{ $errors-> has('id_estadob')?'is-invalid':''}}" name="id_estadob" id="estado" aria-label="Default select example">
		...
		<option value="{{$estado->id_estadob}}" {{ old('id_estadob') == $estado->id_estadob || $admin->id_estadob == $estado->id_estadob ? 'selected' : '' }}>{{$estado->nombre_estadob}}</option>
		@endforeach
	</select>
...
<div class="form-group">
	...
	<select class="form-control {{ $errors-> has('id_lote')?'is-invalid':''}}" name="id_lote" id="lote" value="{{ isset ($admin->id_lote)?$admin->id_lote:old('id_lote') }}" aria-label="Default select example">
		...
		<option value="{{$lote->id_lote}}" {{ old('id_lote') == $lote->id_lote || $admin->id_lote == $lote->id_lote ? 'selected' : '' }}>{{$lote->nombre_lote}}</option>
		@endforeach
	</select>
