...
<div class="form-group">
    ...
    <select class="form-control {{ $errors-> has('id_estadol')?'is-invalid':''}}" name="id_estadol" id="estado" aria-label="Default select example">
        <option value="" selected>Seleccione estado</option>
        @foreach ($estados as $estado)
        <option value="{{$estado->id_estadol}}" {{ ($admin->estado->id_estadol) == $estado->id_estadol ? 'selected' : '' }}>{{$estado->nombre_estadol}}</option>
        @endforeach
    </select>
